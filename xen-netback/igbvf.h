...
#define DASSERT(x) if(!(x)){\
                      printk(KERN_ALERT "DNPMEM ig !!!!!!!!!!!!!!  ASSERT Failure @%s:%d func=%s\n",__FILE__,__LINE__,__func__);\
                      WARN_ON(1);\
                      }

#define DENTER() printk(KERN_INFO "DNPMEM ig %s:%d func=%s | Entry Point \n",__FILE__,__LINE__,__func__)
#define DEXIT() printk(KERN_INFO "DNPMEM ig %s:%d func=%s | Exit Point \n",__FILE__,__LINE__,__func__)
#define DFLOW() printk(KERN_INFO "DNPMEM ig %s:%d func=%s | Flow Point \n",__FILE__,__LINE__,__func__)                            
// dnptwo  >>>>>>>>>>
...
struct igbvf_adapter {
	...
	unsigned long last_reset;
        ...
        /*[DNP] We are introducing some variables for our purpose 
         * of maintaining state of the working of DNP        
         */
        /*Start of change [DNP] 1.1*/       
//#if 1
#ifdef DNP_XEN  
        //To identify which xenvif currently holding this vf
        int dnpvf_id; //if none are associated mark as -1
        
        /*Introduce a function pointer here and initialize to handler during
         *  packet receive by the driver and it need  to send to the netback*/ 
        int (*deliver_packet_to_netback)(struct sk_buff *, struct net_device *, int );
        struct sk_buff *(*alloc_dnpskb)(struct net_device *, unsigned int, int ); //dnptwo
        void (*free_dnpskb)(struct sk_buff *skb, int xenvif_id );
#endif
       /*End of change [DNP] 1.1*/
...
extern void dnp_free_skb(struct sk_buff *, int );
