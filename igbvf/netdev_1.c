/*Kallol: Desc: 
 * [20-03-2014]This file which I edited step by step on the last few days which
 * ended up on some problem .. netdevmodBKP1 and 2 are two files which are
 * interim bkp .. they are fine with making communication as normal igbvf driver
 ***/
/*******************************************************************************
...
struct dnp_cb {
    uint16_t    id; 
    uint32_t gref;  //not required may be ...
    unsigned int *pgad;
    uint32_t handle;
};
                              ...
                              u32 status, u16 vlan)
{
	...
	//netif_receive_skb(skb);
        ...
        /*[DNP][06-10-2013] Commented the above line ... below is my edit
         * Reason: If VF is associated with VM then netback should get the packet
         * otherwise physical machine will get
         */
#ifdef DNP_XEN
        if(adapter->dnpvf_id!=-1){
                DENTER();
                skb_push(skb,ETH_HLEN);  //[DNP-metis][this solved problem]
                adapter->deliver_packet_to_netback(skb,netdev, adapter->dnpvf_id); //need a api for this
        }
        else
#endif
                netif_receive_skb(skb);   
...
/*
#ifdef DNP_XEN //one of the VERY few blind things .. do not why I am using this
         if(adapter->dnpvf_id!=-1){
                                   ...
                                   int cleaned_count)
{
	...
	struct sk_buff *skb;
	unsigned int i;
//        int thricePrint = 0; //dnptwo
	int bufsz;
        //{dnptwo}DO NOT GIVE ANY DENTER OR DFLOW IN THIS ROUTINE IT IS HIGHLY CALLED MODULE
		...
		if (adapter->rx_ps_hdr_size && !buffer_info->page_dma) {
			...
			buffer_info->page_dma =
				dma_map_page(&pdev->dev, buffer_info->page,
					     ...
					     DMA_FROM_DEVICE);
                        //DNP-xen
     //                   printk(KERN_INFO "[DNP-metis]Inside dma-check buffer info:\n ");
    //                    printk(KERN_INFO "[DNP-metis]Page Offset:%d skb = %x\n ",buffer_info->page_offset,buffer_info->skb);
...
// dnptwo  <<<<<<<<<<                    
                  ...
                  /*      if(thricePrint<3){
                                printk(KERN_INFO "DNPMEM ig Thrice Print%d Below\n",thricePrint);
                                DENTER();
                                thricePrint++;
                        }*/
 ...
 *                  skb->len += length;
		     ...
		     skb->truesize += PAGE_SIZE;
                     DEXIT();
                     printk(KERN_INFO "DNPMEM ig skb len=%d, skb_data len=%d, skb_true_size=%d \n", skb->len, skb->data_len, skb->truesize );      
                      DNPMEM ig skb len=60, skb_data len=60, skb_true_size=6272
 */
			     ...
			     struct igbvf_ring *rx_ring)
{
	...
	rx_ring->buffer_info = vzalloc(size);  //[DNP] page Allocation for rx_buffer happening here 
...
//dnptwo pushed the line "if (buffer_info->skb) {" earlier
...
void rx_ring_cleaning(struct igbvf_ring *rx_ring){
    igbvf_clean_rx_ring(rx_ring);
}

EXPORT_SYMBOL(rx_ring_cleaning);
...
//dnptwo or DNP_XEN hint as below::
...
static int __devinit igbvf_sw_init(struct igbvf_adapter *adapter)
{
	struct net_device *netdev = adapter->netdev;
	s32 rc;
        /*[DNP]  setup adapter->dnpvf_id =-1; during initalization 
         * This means this VF has not yet assigned to any netback
         */
        adapter->dnpvf_id=-1;  //[DNP] [06-10-2013]
                                 ...
                                 const struct pci_device_id *ent)
{
	...
	int err, pci_using_dac;   
        ...
        printk(KERN_INFO "[START][DNPMEM] igbvf driver 19.03_1645\n");
...
// dnptwo  <<<<<<<<<<
#ifdef DNP_XEN
        adapter->rx_ring->count = 256; //bad fix .. to help frontend !!
...
#ifdef DNP_XEN
void set_restart_igbvf(struct net_device *dev, int i){       
        struct igbvf_adapter *adapter =NULL;
        DENTER();
...
EXPORT_SYMBOL(set_restart_igbvf);
#endif
